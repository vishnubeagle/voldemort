...
const __movementVector = new THREE.Vector3();
...
export class XrMechanicalControllerInput {
    ...
    /**
     * Handle joystick movement for left, right, forward, and backward directions
     */
    handleJoystickMovement() {
        const movementSpeed = 0.1; // Adjust the speed as needed
    
        __movementVector.set(0, 0, 0);
    
        // Apply joystick movement
        if (this.thumbStick.length() > 0.1) { // Add a dead zone to avoid jitter
            __movementVector.x = this.thumbStick.x * movementSpeed;
            __movementVector.z = -this.thumbStick.y * movementSpeed; // Inverted axis for forward/backward movement
    
            // Apply the movement vector in the direction of the controller's orientation
            this._grip.getWorldQuaternion(__rot);
            __movementVector.applyQuaternion(__rot);
    
            // Update the world position with the movement vector
            this._worldPosition.add(__movementVector);
    
            // Ensure the local position is updated accordingly
            this._localPosition.copy(this._worldPosition);
            this._localPosition.sub(this._grip.parent.position);
            __rot.copy(this._grip.parent.quaternion).invert();
            this._localPosition.applyQuaternion(__rot);
