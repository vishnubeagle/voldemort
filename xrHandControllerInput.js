...
export class XrHandControllerInput {
    ...
    refresh() {
        ...
        // the thumb and the tip of the index finger. It makes the pointer feel
        ...
        // Calculate the shoulder location => __vec3
        ...
        // Use damped values so that a pinch action does not immediately affect the pointer
    ...
    handleJoystickInput() {
        if (this.gamePad && this.gamePad.axes.length >= 2) {
            const xAxis = this.gamePad.axes[2]; // Right joystick horizontal axis
            const yAxis = this.gamePad.axes[3]; // Right joystick vertical axis

            const moveSpeed = 0.05;

            const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(this.head.quaternion);
            const right = new THREE.Vector3(1, 0, 0).applyQuaternion(this.head.quaternion);
            ...
            this.context.camera.position.add(moveDirection);
        }
    }
}
